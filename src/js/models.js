...
const bigGroundConfig = [200, 920, 1700, 180];
const heroStayConfig = [0, 302, 110, 151];
  ...
  render(dt, ctx, canvas) {
    ...
    for (const [index, item] of this.menuItems.entries()) {
      if (index === this.menuIndex) {
        ...
        ctx.fillRect(0, canvas.height / 2 + index * itemHeight - 2, canvas.width, itemHeight);
      }
...
class GameScene {
  constructor(game) {
    ...
    this.heroState = 'stay';
    this.angle = 0;
    this.posX = game.canvas.width * 0.1;
    this.posY = game.canvas.height * 0.7;
  }

  update(dt) {
    if (this.game.keys['87']) this.posY -= this.game.canvas.height * 0.02; // W
  ...
  render(dt, ctx, canvas) {
    ...
    ctx.fillStyle = '#000';
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    ctx.drawImage(bg, 0, 0, (bg.width / bg.height) * canvas.height * 0.8, canvas.height * 0.8);
    ctx.drawImage(fg, ...bigGroundConfig, -50, canvas.height * 0.8, (bg.width / bg.height) * canvas.height * 0.8 + 100, 0.2 * canvas.height);
    //ctx.translate(this.posX, this.posY);
    //ctx.rotate(this.angle * Math.PI / 180);
    ctx.drawImage(hero, ...heroStayConfig, this.posX, this.posY, heroStayConfig[2], heroStayConfig[3]);
  }
}
